...
import org.wa9nnn.fdcluster.model.{ContestProperty, Journal}
...
class JournalPropertySpec extends Specification with Mockito {
  ...
  "JournalManager" >> {
    "initial state" >> {
      val journalManager = new JournalProperty(fileManger, contestProperty, storeSender)
    ...
    "createNewJournal" >> {
      ...
      // reload from file
      val journalManager2 = new JournalProperty(fileManger, contestProperty, storeSender)
